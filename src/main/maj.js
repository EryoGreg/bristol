...
const DEPOT = 'EryoGreg/bristol';
const MOTIF_EXE = /^Bristol-\d+\.\d+\.\d+(-[0-9A-Za-z.]+)?\.exe$/;
...
// a strictement superieure a b ? (semver "x.y.z" ou "x.y.z-test.N")
// A x.y.z egal : une version finale passe devant une pre-version, et deux
// pre-versions se comparent champ par champ (test.2 > test.1).
function versionSuperieure(a, b) {
  const [ca, pra] = String(a).split('-', 2);
  const [cb, prb] = String(b).split('-', 2);
  ...
  const xa = pra.split('.'); const xb = prb.split('.');
  for (let i = 0; i < Math.max(xa.length, xb.length); i++) {
    ...
    if (na && nb && +xa[i] !== +xb[i]) return +xa[i] > +xb[i];
    if (!(na && nb) && xa[i] !== xb[i]) return xa[i] > xb[i];
  }
  return false;
}
...
async function verifier() {
  ...
  try {
    ...
    const res = await net.fetch('https://api.github.com/repos/' + DEPOT + '/releases/latest', {
      headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'Bristol/' + actuelle },
      signal: ctl.signal
    });
